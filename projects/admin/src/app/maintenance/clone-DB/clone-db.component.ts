...
started: boolean = false;
  ...
  // item_count(table_name: string): Observable<number> {
  //   return this.batchService.describeTable(table_name).pipe(
  //     map((description) => {
  //       if (description && description.Table) {
  //         return description.Table.ItemCount || 0;
  //       }
  //       return -1; // Return -1 if the table does not exist or has no item count
  //     })
  //   );
  // }
  ...
  cloneTable(source_table: string) {
    ...
    // this.batchService.scanTable(source_table, 0).pipe(
    //   switchMap((items) => {
    //     return this.batchService.batchWriteItem(this.target_table, items);
    //   })
    // ).subscribe({
    //   next: (data) => {
