...
export class FeesCollectorComponent {
  ...
  tournament_status: 'new' | 'progress' | 'charged' = 'new';
  ...
  async tournament_selected(tournament: club_tournament | null) {
    ...
    try {
    this.tournament_status = await this.feesCollectorService.set_tournament(tournament);


    if (this.tournament_status === 'charged') {
      this.toastService.showInfo('tournoi', 'les droits pour ce tournoi ont déjà été enregistrés !');
    }
    if (this.tournament_status === 'progress') {
      this.toastService.showInfo('tournois', 'un état de saisie précédent a été trouvé pour ce tournoi');
    }
  } catch (error) {
  ...
  check_status() {
    if (this.tournament_status !== 'charged' && this.all_gamers_validated()) {
  ...
  async log_game_state() {
    if(this.tournament_status === 'charged') return;
